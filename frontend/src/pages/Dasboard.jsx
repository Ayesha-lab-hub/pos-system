...
const Dashboard = () => {
  ...
  return (
    <div className="w-full p-6 relative">
      ...
      {/* Timeframe Selector */}
      <div className="flex flex-col md:flex-row bg-white rounded-lg p-4 shadow-sm mb-6 items-center gap-4 border border-gray-100">
        <div className="flex bg-gray-100 rounded-lg p-1">
          {['daily', 'weekly', 'monthly', 'custom'].map(tf => (
            <button
              key={tf}
              onClick={() => setTimeframe(tf)}
              className={`px-4 md:px-6 py-2 rounded-md font-medium capitalize transition-all ${timeframe === tf ? 'bg-white shadow text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
            >
              {tf}
            </button>
          ))}
